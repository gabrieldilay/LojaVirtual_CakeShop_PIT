...
class CupCakeStorePITSupabaseUser extends BaseAuthUser {
  CupCakeStorePITSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> cupCakeStorePITSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = CupCakeStorePITSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
